...
def run_command(cmd):
    ...
    if result.returncode != 0:
        raise RuntimeError("Command execution failed: {0}\n{1}".format(cmd, result.stdout))
    return result.stdout


def build_prefixed_command(*parts):
    return " ".join([part for part in parts if part])


def average_score(runner, runs_per_test):
    scores = []
...
def generate_test_cases_by_trans():
    trans_pairs = [("N", "N"), ("N", "T"), ("T", "N"), ("T", "T")]
    cases_by_trans = {"{0}{1}".format(ta, tb): [] for ta, tb in trans_pairs}
    ...
    for k in k_list:
        for m, n in selected_sizes:
            shape_type = "{0}{1}S".format(label(m), label(n))
            for ta, tb in trans_pairs:
                cases_by_trans["{0}{1}".format(ta, tb)].append(
...
def run_test_case(case, m_vl, n_vl, data_type, prefix):
...
def run_blas_case(case, data_type, prefix):
    m, n, k = case["m"], case["n"], case["k"]
    ta, tb = case["transa"], case["transb"]
    ...
    blas_cmd = build_prefixed_command(
        ...
        "-transa {0}".format(ta),
        "-transb {0}".format(tb),
        "-innerLoops {0}".format(prefix["inner_loops"]),
    )
...
def build_output_path(data_type, output):
    # type: (str, Optional[str]) -> Path
    if output:
        return Path(output)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return SCRIPT_DIR / "sme_kernel_perf_{0}_{1}.csv".format(data_type, timestamp)


def format_improvement(baseline, candidate):
    if baseline <= 0:
        return "N/A"
    improvement = ((candidate / baseline) - 1.0) * 100.0
    return "{0:+.2f}%".format(improvement)


def export_results(results, data_type, output_path):
    ordered_cols = [
        "Command",
        "Thread",
        "Function",
    ...
    print("[File] {0} result saved to: {1}".format(data_type, output_path))
...
def main():
    ...
    print("=" * 100)
    print("  SME Kernel vs GotoBLAS ({0})".format(data_type))
    print("=" * 100)

    all_cases = generate_test_cases_by_trans()
    total_cases = len(all_cases)
    print("Total test cases: {0}".format(total_cases))
    ...
    for case in all_cases:
        m, n, k = case["m"], case["n"], case["k"]
        ta, tb = case["transa"], case["transb"]
        cmd_str = "{0} -transa {1} -transb {2} -m {3} -n {4} -k {5}".format(
            blas_binary, ta, tb, m, n, k
        )
        results.append(
            {
                "Command": cmd_str,
    ...
    try:
        ...
        for i, case in enumerate(all_cases):
            print("[{0}/{1}] BLAS: {2:60} ... ".format(i + 1, total_cases, results[i]["Command"]), end="", flush=True)
        ...
        for tile_name, m_vl, n_vl in CONFIG["kernel_shapes"]:
            print("\n" + "=" * 100)
            print(">>> Starting tile test: {0} (m_vl={1}, n_vl={2}) <<<".format(tile_name, m_vl, n_vl))
            for i, case in enumerate(all_cases):
                print("[{0}/{1}] {2:8}: {3:60} ... ".format(i + 1, total_cases, tile_name, results[i]["Command"]), end="", flush=True)
                score = run_test_case(case, m_vl, n_vl, data_type, prefix)
                results[i]["Mflops_{0}".format(tile_name)] = score
                print("{0:,.2f}".format(score))
